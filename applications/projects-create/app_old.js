...
function wsPageForm_112346_01(){
	_wsProgress(80);
	$.getJSON('https://script.google.com/macros/s/AKfycbyBeaAyqxk2iphU72PsWmmHein8fwd17s5a0EKPhXiYVNYpM88/exec?type=create&values=["'+window.currentDataItem2+'", "'+encodeURIComponent($("#u_Title").val())+'", "'+encodeURIComponent($("#u_Description").val())+'", "'+$("#u_Type").val()+'", "'+$("#u_Category").val()+'", "'+encodeURIComponent($("#u_Tag").val())+'", "'+nowTime()+'", "'+$("#u_End").val()+'", "'+$("#u_UOW").val()+'", "'+$("#u_Price").val()+'", "'+window.authUser[0].CURR+'", "AUTO", "AUTO", "Submitted", "", "*_'+window.authUser[0].sl+'_", "'+window.authUser[0].Name+'"]').done(function(result){
		_wsProgress(100);
		if(result){
