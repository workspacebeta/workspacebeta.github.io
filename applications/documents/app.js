...
if(window.authUser[0].Role == 'Power') userPayslipAccess = "";
	...
	$("#ws-ui-113762-1").kendoGrid({
		dataSource: {
			...
			transport: {
				read: {
					url: "https://script.google.com/macros/s/AKfycby4yFy3bKYha9u4dcDfWbAzHWuNiHLkjrNSRmiMlldLGjWWcRI/exec?type=grid&rl="+window.authUser[0].Role+userPayslipAccess
				}
			},
		...
		columns: [
			...
			{field:"Refno", title: "Reference No", width: 180},
			{command: {text: "View", click: ws_ui_113762_btn1}, title: " ", width: "100px"},
			{field:"DocDate", title: "Date", width: 180, template: '#= kendo.toString(DocDate, "dd/MM/yyyy") #'}
		] 
	});
	...
	function ws_ui_113762_btn1(e){
		...
		currentDataDoc.push(dataItem.Duration);
		currentDataDoc.push(dataItem.Time);
		currentDataDoc.push(dataItem.WarningReasons);
		currentDataDoc.push(dataItem.PreviousRefno);
		currentDataDoc.push(dataItem.TrainingBatch);
		currentDataDoc.push(dataItem.Simno);
		currentDataDoc.push(dataItem.SimEffectiveDate);
		currentDataDoc.push(dataItem.SimPlan);
		currentDataDoc.push(dataItem.PolicyNo);
		currentDataDoc.push(dataItem.PolicyAmount);
		currentDataDoc.push(dataItem.PolicyIssuedOn);
		currentDataDoc.push(dataItem.PolicyExpireOn);
