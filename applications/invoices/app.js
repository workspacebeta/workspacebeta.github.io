...
if(typeof window.authUser[0]["navigation"][3] != 'undefined') if(window.authUser[0]["navigation"][3] != '') projectsInvoiceAccess = "";
	...
	$("#ws-ui-113762-1").kendoGrid({
		dataSource: {
			...
			transport: {
				read: {
					url: "https://script.google.com/macros/s/AKfycbySbqmv1Osz21CDA4v1oYb_feBhUCAF8ECZHo-wAADWyNjp7yxb/exec?type=grid&rl="+window.authUser[0].Role+projectsInvoiceAccess
				}
			},
	...
	function ws_ui_113762_btn2(e){
		...
		$("#ws-112346-01").click(function(){
			_wsProgress(80);
			$.getJSON('https://script.google.com/macros/s/AKfycbz64XTE-Yu1m5pRD3nMxFF-9yhM6V-IJ3q62Pxw6gjOdOIi9H3W/exec?type=create&cpam='+dataItem.Paid+'&sl='+dataItem.sl+'&values=["AUTO", "'+window.authUser[0].sl+'", "'+window.authUser[0].Name+'", "'+dataItem.ID+'", "'+dataItem.CURR+'", "0", "'+$('#u_Amount').val()+'", "Pending", "Project Invoice", "PayPal"]').done(function(result){
				if(result){
