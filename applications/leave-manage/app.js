$("#ws-explore").html('<div class="row"><div class="col-md-12"><h2>Manage Leaves</h2><p>Manage leaves by taking actions.</p></div></div><div class="row"><div class="col-md-12"><div id="ws-ui-112346" class="bottom-20"></div><div class="bottom-20"><button type="button" class="k-button k-primary" id="ws-ui-btn1">Approve</button> <button type="button" class="k-button" id="ws-ui-btn2">Cancel</button></div></div></div>');
	...
	$("#ws-ui-112346").kendoGrid({
		dataSource: {
			...
			transport: {
				read: {
					url: "https://script.google.com/macros/s/AKfycby0hDsGGPj9f5NweVbeFI3mu6nOosS1KuaZbAt6FQloFCm1eq4/exec?type=grid&rl="+window.authUser[0].Role+"&ol=Leave"
				}
			},
...
function userAction(sl, status){
	...
	$.getJSON('https://script.google.com/macros/s/AKfycby0hDsGGPj9f5NweVbeFI3mu6nOosS1KuaZbAt6FQloFCm1eq4/exec?type=update&rl='+window.authUser[0].Role+'&sl='+sl+'&values=["'+status+'", "'+window.authUser[0].sl+'", "'+window.authUser[0].Name+'", "'+nowTime()+'"]').done(function(result){
		if(result){
