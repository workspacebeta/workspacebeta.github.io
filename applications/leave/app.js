...
function wsPageForm_112346_01(){
	_wsProgress(80);
	$.getJSON('https://script.google.com/macros/s/AKfycby0hDsGGPj9f5NweVbeFI3mu6nOosS1KuaZbAt6FQloFCm1eq4/exec?type=create&ol=Leave&values=["'+window.authUser[0].sl+'", "'+window.authUser[0].Name+'", "'+nowTime()+'", "'+$("#u_For").val()+'", "Leave", "'+escape($("#u_Description").val())+'", "'+window.authUser[0].Location+'", "Request", "", "", ""]').done(function(result){
		_wsProgress(100);
		if(result){
	...
	$("#ws-ui-112346").kendoGrid({
		dataSource: {
			...
			transport: {
				read: {
					url: "https://script.google.com/macros/s/AKfycby0hDsGGPj9f5NweVbeFI3mu6nOosS1KuaZbAt6FQloFCm1eq4/exec?type=grid&rl="+window.authUser[0].Role+"&ol=Leave&usersl="+window.authUser[0].sl
				}
			},
