$("#ws-explore").html('<div class="row"><div class="col-md-12"><h2>Rework Projects</h2><p>All rework projects available for you. Only archived items are not visable.</p></div></div><div class="row"><div class="col-md-12"><div id="ws-ui-112345"></div><div id="ws-ui-112346"><div id="ws-ui-112346-1" class="bottom-20"></div><div id="ws-ui-112346-2" class="bottom-20"></div></div></div></div>');
...
_uiGrid(
	'ws-ui-112345', 
	"https://script.google.com/macros/s/AKfycbz_CLTkzYCuC1ZtnXkAjAm0kPzvZ-Zm2XLQezQABINdUessxhZn/exec?type=grid&rl="+window.authUser[0].Role+projectsGridAccess+"&clients=["+window.authUser[0].Clients+"]", 
	"https://script.google.com/macros/s/AKfycbz_CLTkzYCuC1ZtnXkAjAm0kPzvZ-Zm2XLQezQABINdUessxhZn/exec", 
	550, 20, {
		sl: {type: "number"}, 
		ID: {type: "string"}, 
	...
	[
		...
		{field:"Price", title: "Price", width: 100, aggregates: ["sum"], footerTemplate: "Total: #=sum#"},
...
_uiGrid(
	...
	[
		...
		{field:"UserName", title: "Client", width: 110},
...
function wsUIButton_112302(e){
	...
	if(dataItem.Status != 'Invoiced'){
		...
		window.currentDataItemEditURL = 'https://script.google.com/macros/s/AKfycbz_CLTkzYCuC1ZtnXkAjAm0kPzvZ-Zm2XLQezQABINdUessxhZn/exec';
		window.currentDataItem18 = 'projects-2';
...
function wsUIButton_112300(e){
	...
	$("#ws-ui-113753").kendoGrid({
		dataSource: {
			...
			transport: {
				read: {
					url: "https://script.google.com/macros/s/AKfycbz_CLTkzYCuC1ZtnXkAjAm0kPzvZ-Zm2XLQezQABINdUessxhZn/exec?type=details&rl="+window.authUser[0].Role+"&metadata="+dataItem.Metadata+"&nocomments=true"
				}
			},
	...
	$("#ws-ui-113754").kendoDropDownList({
		...
		dataSource: {
			...
			transport: {
				read: {			
				url: "https://script.google.com/macros/s/AKfycbz_CLTkzYCuC1ZtnXkAjAm0kPzvZ-Zm2XLQezQABINdUessxhZn/exec?type=progress-actions&rl="+window.authUser[0].Role+"&metadata="+dataItem.Metadata,
				}
			}
		},
		select: function(e){
	...
	$("#ws-ui-113761").kendoGrid({
		dataSource: {
			...
			transport: {
				read: {
					url: "https://script.google.com/macros/s/AKfycbyYIM7oU0YiD4IqT6ElvunWOpeXMaZgV2WPEFnMeUOF7EJ1r-3A/exec?type=grid&rl="+window.authUser[0].Role+"&service="+dataItem.ID+"&ol=Allocation"
				}
			},
		...
		change: function(e){
			...
			$("#ws-ui-113763").kendoGrid({
				dataSource: {
					...
					transport: {
						read: {
							url: "https://script.google.com/macros/s/AKfycbyYIM7oU0YiD4IqT6ElvunWOpeXMaZgV2WPEFnMeUOF7EJ1r-3A/exec?type=skill&rl="+window.authUser[0].Role+"&service="+dataItem.ID+"&ol=Allocation&date="+udate
						}
					},
					schema:{
						model: {
			...
			$("#ws-ui-113764").kendoGrid({
				dataSource: {
					...
					transport: {
						read: {
							url: "https://script.google.com/macros/s/AKfycbyYIM7oU0YiD4IqT6ElvunWOpeXMaZgV2WPEFnMeUOF7EJ1r-3A/exec?type=allocation-report&rl="+window.authUser[0].Role+"&date="+udate
						}
					},
					schema:{
						model: {
	...
	$("#ws-ui-113761 .k-grid-content").on("change", "input.wsUIChkbox113761", function(e){
		...
		$.getJSON("https://script.google.com/macros/s/AKfycbyYIM7oU0YiD4IqT6ElvunWOpeXMaZgV2WPEFnMeUOF7EJ1r-3A/exec?type="+type+"&rl="+window.authUser[0].Role+"&service="+dataItem.ID+"&usersl="+dataItem.UserSl).done(function(result){
			if(result){
				$("#ws-ui-113761").data('kendoGrid').dataSource.read();
			}else{
...
function action_ws_ui_100001_1(){
	_wsProgress(80);
	$.getJSON("https://script.google.com/macros/s/AKfycbz_CLTkzYCuC1ZtnXkAjAm0kPzvZ-Zm2XLQezQABINdUessxhZn/exec?type=details&rl="+window.authUser[0].Role+"&metadata="+window.currentDataItem.Metadata).done(function(result){
		if(result){
			$("#ws-ui-100002-1").html('');			
...
function action_ws_ui_100001_2(){
	_wsProgress(80);
	$.getJSON("https://script.google.com/macros/s/AKfycbz_CLTkzYCuC1ZtnXkAjAm0kPzvZ-Zm2XLQezQABINdUessxhZn/exec?type=details&rl="+window.authUser[0].Role+"&metadata="+window.currentDataItem.Metadata+"&queryonly=true").done(function(result){
		if(result){
			$("#ws-ui-100002-2").html('');			
			for(var ws_i = result.length - 1; ws_i >= 0; ws_i--){
				if(window.authUser[0].sl == result[ws_i]['UserSl']){
