...
_uiGrid(
	'ws-ui-112345', 
	"https://script.google.com/macros/s/AKfycbyBeaAyqxk2iphU72PsWmmHein8fwd17s5a0EKPhXiYVNYpM88/exec?type=grid&rl="+window.authUser[0].Role+projectsGridAccess+"&clients=["+window.authUser[0].Clients+"]", 
	"https://script.google.com/macros/s/AKfycbyBeaAyqxk2iphU72PsWmmHein8fwd17s5a0EKPhXiYVNYpM88/exec", 
	550, 20, {
		sl: {type: "number"}, 
		ID: {type: "string"}, 
...
function wsUIButton_112302(e){
	...
	if(dataItem.Status != 'Invoiced'){
		...
		window.currentDataItemEditURL = 'https://script.google.com/macros/s/AKfycbyBeaAyqxk2iphU72PsWmmHein8fwd17s5a0EKPhXiYVNYpM88/exec';
...
function wsUIButton_112300(e){
	...
	$("#ws-ui-113753").kendoGrid({
		dataSource: {
			...
			transport: {
				read: {
					url: "https://script.google.com/macros/s/AKfycbyBeaAyqxk2iphU72PsWmmHein8fwd17s5a0EKPhXiYVNYpM88/exec?type=details&rl="+window.authUser[0].Role+"&metadata="+dataItem.Metadata+"&nocomments=true"
				}
			},
	...
	$("#ws-ui-113754").kendoDropDownList({
		...
		dataSource: {
			...
			transport: {
				read: {			
				url: "https://script.google.com/macros/s/AKfycbyBeaAyqxk2iphU72PsWmmHein8fwd17s5a0EKPhXiYVNYpM88/exec?type=progress-actions&rl="+window.authUser[0].Role+"&metadata="+dataItem.Metadata,
				}
			}
		},
		select: function(e){
	...
	$("#ws-ui-113761").kendoGrid({
		dataSource: {
			...
			transport: {
				read: {
					url: "https://script.google.com/macros/s/AKfycbzVkMNMk7TfEm5m_jj0mFD3P0s4Thx4Gbk27jVS9PwxGvzi02uW/exec?type=grid&rl="+window.authUser[0].Role+"&service="+dataItem.ID+"&ol=Allocation"
				}
			},
		...
		change: function(e){
			...
			$("#ws-ui-113763").kendoGrid({
				dataSource: {
					...
					transport: {
						read: {
							url: "https://script.google.com/macros/s/AKfycbzVkMNMk7TfEm5m_jj0mFD3P0s4Thx4Gbk27jVS9PwxGvzi02uW/exec?type=skill&rl="+window.authUser[0].Role+"&service="+dataItem.ID+"&ol=Allocation&date="+udate
						}
					},
					schema:{
						model: {
			...
			$("#ws-ui-113764").kendoGrid({
				dataSource: {
					...
					transport: {
						read: {
							url: "https://script.google.com/macros/s/AKfycbzVkMNMk7TfEm5m_jj0mFD3P0s4Thx4Gbk27jVS9PwxGvzi02uW/exec?type=allocation-report&rl="+window.authUser[0].Role+"&date="+udate
						}
					},
					schema:{
						model: {
	...
	$("#ws-ui-113761 .k-grid-content").on("change", "input.wsUIChkbox113761", function(e){
		...
		$.getJSON("https://script.google.com/macros/s/AKfycbzVkMNMk7TfEm5m_jj0mFD3P0s4Thx4Gbk27jVS9PwxGvzi02uW/exec?type="+type+"&rl="+window.authUser[0].Role+"&service="+dataItem.ID+"&usersl="+dataItem.UserSl).done(function(result){
			if(result){
				$("#ws-ui-113761").data('kendoGrid').dataSource.read();
			}else{
...
function action_ws_ui_100001_1(){
	_wsProgress(80);
	$.getJSON("https://script.google.com/macros/s/AKfycbyBeaAyqxk2iphU72PsWmmHein8fwd17s5a0EKPhXiYVNYpM88/exec?type=details&rl="+window.authUser[0].Role+"&metadata="+window.currentDataItem.Metadata).done(function(result){
		if(result){
			$("#ws-ui-100002-1").html('');			
...
function action_ws_ui_100001_2(){
	_wsProgress(80);
	$.getJSON("https://script.google.com/macros/s/AKfycbyBeaAyqxk2iphU72PsWmmHein8fwd17s5a0EKPhXiYVNYpM88/exec?type=details&rl="+window.authUser[0].Role+"&metadata="+window.currentDataItem.Metadata+"&queryonly=true").done(function(result){
		if(result){
			$("#ws-ui-100002-2").html('');			
			for(var ws_i = result.length - 1; ws_i >= 0; ws_i--){
				if(window.authUser[0].sl == result[ws_i]['UserSl']){
